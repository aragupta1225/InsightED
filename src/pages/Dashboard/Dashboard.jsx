              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-border-subtle bg-paper/50">
                    ...
                    <th className="px-6 py-3 font-semibold text-text-secondary text-sm">Class</th>
                ...
                <tbody>
                  {attentionStudents.map((item, idx) => (
                    <tr key={idx} className="border-b border-border-subtle last:border-none hover:bg-paper-light transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-text-secondary">{item.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {attentionClasses.map((item, idx) => (
                    <tr key={idx} className="border-b border-border-subtle last:border-none hover:bg-paper-light transition-colors">
                      <td className="px-6 py-4 font-semibold text-navy">{item.class}</td>
                      <td className="px-6 py-4 text-sm text-text-secondary">{item.reason}</td>
                    </tr>
                  ))}
                </tbody>
            ...
            {isAddingTask ? (
              <form onSubmit={addTask} className="mt-auto flex flex-col gap-3 p-4 bg-paper-light rounded-xl border border-border-subtle">
                ...
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setIsAddingTask(false)}>Cancel</Button>
                  <Button type="submit" variant="primary" className="px-4 py-1.5 text-xs">Save Task</Button>
                </div>
              </form>
            ) : (
              <Button 
                variant="outline" 
                className="w-full mt-auto border-dashed border-2 text-text-secondary hover:text-navy hover:border-navy"
